...
ml <- glm(count ~ poem, family = poisson, data = poem)
summary(ml)
multinom(poem ~ 1, data = poem, weights = count)

coef_poisson <- coef(ml)
coef_multinom <- coefficients(multinom(poem ~ 1, data = poem, weights = count))
```
```{r}
