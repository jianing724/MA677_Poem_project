...
```{r}
## poisson regression
poem <- read.csv("poem_project.csv")
...
```
```{r}
library(nnet)  # Load the nnet package for multinom

# Read the data
poem <- read.csv("poem_project.csv")

# Categorize the count variable
poem$count_cat <- cut(poem$count,
                      breaks = quantile(poem$count, probs = c(0, 1/3, 2/3, 1), na.rm = TRUE),
                      labels = c("Low", "Medium", "High"),
                      include.lowest = TRUE)

# Convert categorical variables to factors
poem$count_cat <- as.factor(poem$count_cat)
poem$poem6 <- as.factor(poem$poem6)
poem$end <- as.factor(poem$end)
poem$followed <- as.factor(poem$followed)

# Fit the multinomial model
model_multinom <- multinom(count_cat ~ poem2 + poem3 + poem4 + poem5 + poem6 + end + followed + poem6:end + poem6:followed, data = poem)
